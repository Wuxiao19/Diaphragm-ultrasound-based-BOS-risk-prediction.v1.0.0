...
class DetectionPipeline:
    ...
    def load_models(self):
        ...
        if not os.path.exists(feature_order_path):
            raise FileNotFoundError(f"ExtraTrees feature order file not found: {feature_order_path}")
    ...
    def reduce_features(self, features, filenames, model_type):
    ...
    def reduce_features_from_df(self, df_raw, model_type):
        ...
        if missing:
            raise ValueError(f"{model_type} CSV missing feature columns {missing[:5]} (total {len(missing)})")
        ...
        self.log(f"CSV feature reduction done: original {df_raw.shape[1]} columns -> {len(feature_cols)} selected")
        return df

    def merge_features(self, df_b, df_m):
        """Merge B- and M-mode features by (date, patient_id) parsed from filenames"""
        self.log("Start merging B- and M-mode features...")

        def extract_date_pid(name):
            match = re.match(r"(\d{2}-\d{2}-\d{2})-(C\d{3}|B\d{3}|P\d{3})", name)
            if match:
    ...
    def save_results(self, merged_df, probabilities, predictions, output_dir):
            ...
            if results_df[group_key].duplicated().any():
                ...
                for name, group in results_df.groupby(group_key):
                    ...
                    aggregated_rows.append({
                        ...
                        'b_filename': ";".join(group['b_filename'].astype(str).unique()),
                        'm_filename': ";".join(group['m_filename'].astype(str).unique()),
        ...
        if hasattr(self, 'missing_modalities') and self.missing_modalities:
            ...
            self.log(f"Found {len(missing_df)} samples missing B or M modality; saved to: {missing_csv_path}")
        else:
    ...
    def run(self, b_input, m_input, is_folder, use_csv):
        """Run full pipeline"""
        try:
            ...
            if use_csv:
                ...
                self.log(f"B CSV samples: {len(df_b_raw)}, M CSV samples: {len(df_m_raw)}")
            else:
                if is_folder:
                    b_paths = sorted([os.path.join(b_input, f) for f in os.listdir(b_input)
                                      if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp'))])
                    m_paths = sorted([os.path.join(m_input, f) for f in os.listdir(m_input)
                                      if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp'))])
                else:
                    b_paths = [b_input]
                    m_paths = [m_input]
...
def ensure_checkpoints_available(
    ...
    def merge_features(self, df_b, df_m):
        ...
        # Parse filenames to get date and patient ID
        def extract_date_pid(name):
            match = re.match(r"(\d{2}-\d{2}-\d{2})-(C\d{3}|B\d{3}|P\d{3})", name)
            if match:
                return match.group(1), match.group(2)
        ...
        # ==========================================================
        # Collect (date, pid) pairs existing only in B or only in M
        # ==========================================================
        ...
        for d, p in sorted(only_b):
            missing_records.append({
                'date': d,
                'pid': p,
                'missing_modality': 'missing M modality'
            })
        ...
        for d, p in sorted(only_m):
            missing_records.append({
                'date': d,
                'pid': p,
                'missing_modality': 'missing B modality'
            })

        # Save into pipeline instance for later use
        self.missing_modalities = missing_records
        ...
        for idx_m, row_m in df_m.iterrows():
            ...
            m_filename = row_m['filename']
                    ...
                    # Find all matching B-mode rows
            candidates = df_b[(df_b['date'] == m_date) & (df_b['pid'] == m_pid)]
            
            if len(candidates) > 0:
                for idx_b, row_b in candidates.iterrows():
                    b_filename = row_b['filename']
                    # Use common prefix (date + patient_id) as merged filename
                    merged_filename = f"{m_date}-{m_pid}"
    ...
    def predict(self, merged_df):
        ...
        # ==========================================================
        # Build X using the exact same feature order as training
        # ==========================================================
        missing_cols = [c for c in self.et_feature_order if c not in merged_df.columns]
        if missing_cols:
            raise RuntimeError(
    ...
    def save_results(self, merged_df, probabilities, predictions, output_dir):
        ...
        if len(results_df) > 1:
            ...
            # Check if there are duplicated date-pid keys
            if results_df[group_key].duplicated().any():
                self.log("Found multiple rows with the same date and patient_id, start aggregating...")
                ...
                for name, group in results_df.groupby(group_key):
                    ...
                    # Majority vote (if tie, pandas.mode() returns multiple, take the first)
                    mode_pred = group['prediction'].mode().iloc[0]
                    mode_label = 'diseased' if mode_pred == 1 else 'healthy'
        ...
        # ==========================================================
        # Save samples with missing B / M modality
        # ==========================================================
    ...
    def run(self, b_input, m_input, is_folder, use_csv):
        ...
        try:
            ...
            # 1/2. Obtain features (from images or CSV)
            if use_csv:
                self.log("Using feature-CSV mode")
                df_b_raw = pd.read_csv(b_input)
                df_m_raw = pd.read_csv(m_input)
                ...
                # Prepare image paths (sorted to be deterministic)
                if is_folder:
                    b_paths = sorted([os.path.join(b_input, f) for f in os.listdir(b_input) 
                              if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp'))])
                    m_paths = sorted([os.path.join(m_input, f) for f in os.listdir(m_input) 
            ...
            # 6. Save results
            result_csv_path, results_df = self.save_results(merged_df, probabilities, predictions, output_dir)
