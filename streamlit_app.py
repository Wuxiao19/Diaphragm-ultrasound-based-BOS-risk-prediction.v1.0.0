...
if "last_results_df" in st.session_state and "last_output_dir" in st.session_state:
    ...
    # ============================================================
    st.markdown("---")
    st.subheader("4. AI 解读（Qwen3-8B）")
    st.caption(
        "说明：你可以选择两种模式："
        "1) **传统模式**：基于已有检测结果做中文解释；"
        "2) **Agent 模式**：让 Qwen 自己调用检测工具并给出完整分析（推荐）。"
        "该解读不能替代医生最终诊断。"
    )
    ...
    ai_mode = st.radio(
        "选择 AI 解读模式",
        options=["agent", "explain"],
        format_func=lambda x: "🤖 Agent 模式（让 Qwen 自己调用工具）" if x == "agent" else "📝 传统模式（基于已有结果解释）",
        horizontal=True,
    )
    ...
    if ai_mode == "agent":
        # Agent 模式：让 Qwen 自己调用工具
        st.info("🤖 **Agent 模式**：Qwen 会根据上传的图片路径，自动选择并调用检测工具，然后给出完整分析。")

        # 从 session_state 获取上传文件的路径（如果还在）
        b_path_for_agent = None
        m_path_for_agent = None
        b_folder_for_agent = None
        m_folder_for_agent = None

        # 尝试从 session_state 获取路径（需要保存）
        if "last_b_path" in st.session_state:
            b_path_for_agent = st.session_state["last_b_path"]
        if "last_m_path" in st.session_state:
            m_path_for_agent = st.session_state["last_m_path"]
        if "last_b_folder" in st.session_state:
            b_folder_for_agent = st.session_state["last_b_folder"]
        if "last_m_folder" in st.session_state:
            m_folder_for_agent = st.session_state["last_m_folder"]
        ...
        if st.button("🚀 启动 Qwen Agent（自动调用检测工具）", type="primary"):
            ...
            elif not b_path_for_agent and not b_folder_for_agent:
                st.warning("⚠️ 未找到图片路径信息。请先完成步骤 2（Run inference），然后再使用 Agent 模式。")
            else:
                try:
                    with st.spinner("🤖 Qwen Agent 正在工作：调用检测工具并生成分析..."):
                        ...
                        if b_path_for_agent and m_path_for_agent:
                            ...
                            raise ValueError("无法确定是单组还是批量模式")
        ...
        # 传统模式：基于已有结果解释
        st.info("📝 **传统模式**：基于步骤 2 的检测结果，生成中文解释。")
        ...
        if st.button("📝 生成 AI 解读（传统模式）", type="secondary"):
            if not qwen_api_key.strip():
                st.error("请先填写 Qwen API Key（或在系统环境变量里设置 QWEN_API_KEY）。")
            else:
                ...
                try:
                    ...
                    with st.spinner("正在调用 Qwen 生成中文解读，请稍等..."):
                        explanation = qwen_explain_detection_sync(
